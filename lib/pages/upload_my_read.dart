...
class UploadMyReadScreen extends StatefulWidget {
  ...
  const UploadMyReadScreen({Key key,this.customerId,this.monthName}) : super(key: key);
...
Future<void> upload(MyReadUnit model) async {
  if (FirebaseAuth.instance.currentUser?.uid != null) {
    ...
    await FirebaseFirestore.instance.collection(userUploadUnitCollection).doc(FirebaseAuth.instance.currentUser.uid).collection(userReadUnitCollection).doc(monthName.replaceAll("/", "-")+'-'+widget.customerId)
  ...
  Future<bool> checkUnitExist(bool instail) async {
    var value=  await FirebaseFirestore.instance.collection(userUploadUnitCollection).doc(FirebaseAuth.instance.currentUser.uid).collection(userReadUnitCollection).doc(monthName.replaceAll("/", "-")+'-'+widget.customerId).get();
  ...
  Widget _previewImages() {
        ...
        children: [
          ...
          Container(
           // label: 'image_picker_example_picked_image',
            child: kIsWeb
                ? Image.network(file!=null?file.path:myReadUnit.readImageUrl)
                :  Container(
    ...
    _progress.expectedTotalBytes,
              ...
              imageProvider:file!=null? FileImage(File(file.path)):NetworkImage(myReadUnit.readImageUrl),
            ),
                ),
          ),
          Positioned(
              top: 5,
          right: 5,
              child: IconButton(onPressed: chooseImage, icon: Icon(Icons.camera,color: Colors.red,size: 40,))),
    ...
    Container(
        ...
        ),child: SpinKitCubeGrid(color: Colors.white,size: 100,)):Container()
      ...
      return   TextButton.icon(
       ...
       // label: 'Take picture of read unit.',,
        ...
        icon: Icon(Icons.camera,color: Colors.red,size: 30,),
      );
    }
  }
  ...
  Future<firebase_storage.UploadTask> uploadFile(PickedFile file) async {
    ...
    firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance
        ...
        .child('/'+monthName.replaceAll("/", "-")+'-'+meter.customerId+'.jpg');
  ...
  Future<bool> checkMeterExist() async {
    var value=  await FirebaseFirestore.instance.collection(meterCollection).doc(FirebaseAuth.instance.currentUser.uid).collection(userMeterCollection).doc(widget.customerId).get();
    ...
    if(value.exists){
     ...
     myMsg.MessageHandler.showMessage(context, "", "this device is not connected to your account");
     setState(() {
       _isLoading=false;
     });
      return false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading?(SpinKitDoubleBounce(color: Colors.red,)): SingleChildScrollView(
        child: Container(
          color: Colors.white,
            ...
            children: <Widget>[
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Text("For: "+monthName,style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20),),

                    Text(myReadUnit!=null && myReadUnit.status.length>0?myReadUnit.status:"",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 14,color: Colors.red),),
                  ],
                ),),
              Container(
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _isUploadingData?SpinKitDoubleBounce(color: Colors.pink,):  ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        padding: EdgeInsets.symmetric(horizontal: 20, vertical: 15),
                      ),
                      onPressed: () async {
                        if(file==null&& (myReadUnit==null ||myReadUnit.readImageUrl.length<1)){
                          myMsg.MessageHandler.showErrMessage(context, "Picture is required", "Plase take a picture and upload");
                          return;
                        }
                        if(_profileformKey.currentState.validate()) {
                        bool existAndApproved=await  checkUnitExist(false);
                        if(existAndApproved){
                          myMsg.MessageHandler.showErrMessage(context, "Can not edit", monthName+"\' s unit is been approved" );
                          return;
                        }
                        setState(() {
                          ...
                          model.readDate = Timestamp.fromDate(DateTime.now());
                          model.companyId = "1";
                          if(file!=null) {
                            setState(() {
                          ...
                          }else if(myReadUnit!=null && myReadUnit.readImageUrl.length>0){
                            model.readImageUrl=myReadUnit.readImageUrl;
                            upload(model);
                          }
